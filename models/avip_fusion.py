...
class VisualEncoder(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(64 * 640 * 192, 64)
        self.fc_cov = nn.Linear(64 * 640 * 192, 64)
        self.relu = nn.ReLU()
...
class SensorFusion(nn.Module):
    ...
    def forward(self, inertial_feature, visual_feature, dvl_feature, pressure_feature):
        x = torch.cat((inertial_feature, visual_feature, dvl_feature, pressure_feature), dim=1)
...
class NeuralTransitionGeneration(nn.Module):
    def __init__(self):
        ...
        self.fc_A = nn.Linear(256, 256 * 256)
        self.fc_cov = nn.Linear(256, 256 * 256)
        self.relu = nn.ReLU()

    def forward(self, latent_state):
        x = self.relu(self.fc1(latent_state))
        x = self.relu(self.fc2(x))
        transition_matrix = self.fc_A(x).view(-1, 256, 256)
        process_noise_cov = self.fc_cov(x).view(-1, 256, 256)
...
class UnderwaterLocalizationSystem(nn.Module):
    ...
    def forward(self, imu_data, dvl_data, visual_data, pressure_data, prev_latent_states, prev_state_cov):
        # Encode sensor data
        inertial_feature, inertial_noise_cov = self.inertial_encoder(imu_data)
        dvl_feature, dvl_noise_cov = self.dvl_encoder(dvl_data)
        visual_feature, visual_noise_cov = self.visual_encoder(visual_data)
        pressure_feature, pressure_noise_cov = self.pressure_encoder(pressure_data)

        # Fuse sensor features
        observation = self.sensor_fusion(inertial_feature, visual_feature, dvl_feature, pressure_feature)
...
# system = UnderwaterLocalizationSystem()

# Generate random data
# batch_size = 8
# imu_data = torch.randn(batch_size, 10, 6)  # Shape (B, 10, 6)
# dvl_data = torch.randn(batch_size, 3)  # Shape (B, 3)
# visual_data = torch.randn(batch_size, 6, 640, 192)  # Shape (B, 6, 640, 192)
# pressure_data = torch.randn(batch_size, 1)  # Shape (B, 1)
# prev_latent_states = torch.randn(batch_size, 256)  # Shape (B, 256)
# prev_state_cov = torch.eye(256).unsqueeze(0).repeat(batch_size, 1, 1)  # Shape (B, 256, 256)
# observation_noise = torch.eye(256).unsqueeze(0).repeat(batch_size, 1, 1)  # Shape (B, 256, 256)

# Forward pass
# poses, current_latent_states, current_state_cov = system(
#     imu_data, dvl_data, visual_data, pressure_data, prev_latent_states, prev_state_cov
# )

# print("Poses:", poses)
# print("Current Latent States:", current_latent_states)
# print("Current State Covariance:", current_state_cov)
...
for epoch in range(num_epochs):
    model.train()
    running_loss = 0.0
    for i, (imu_data, dvl_data, visual_data, pressure_data, ground_truth_poses) in enumerate(train_loader):
        # Zero the parameter gradients
        optimizer.zero_grad()

        # Forward pass
        prev_latent_states = torch.randn(batch_size, 256)
        prev_state_cov = torch.eye(256).unsqueeze(0).repeat(batch_size, 1, 1)
        observation_noise = torch.eye(256).unsqueeze(0).repeat(batch_size, 1, 1)
        poses, _, _ = model(imu_data, dvl_data, visual_data, pressure_data, prev_latent_states, prev_state_cov)
    ...
    val_loss = 0.0
    with torch.no_grad():
        for imu_data, dvl_data, visual_data, pressure_data, ground_truth_poses in val_loader:
            prev_latent_states = torch.randn(batch_size, 256)
            prev_state_cov = torch.eye(256).unsqueeze(0).repeat(batch_size, 1, 1)
            observation_noise = torch.eye(256).unsqueeze(0).repeat(batch_size, 1, 1)
            poses, _, _ = model(imu_data, dvl_data, visual_data, pressure_data, prev_latent_states, prev_state_cov)
