...
imu_train = imu_data[:split_idx]
dvl_train = dvl_data[:split_idx]
pressure_train = pressure_data[:split_idx]
ground_truth_train = ground_truth[:split_idx]

imu_eval = imu_data[split_idx:]
dvl_eval = dvl_data[split_idx:]
pressure_eval = pressure_data[split_idx:]
ground_truth_eval = ground_truth[split_idx:]
...
imu_train_mean = np.mean(imu_train, axis=0)
imu_train_std = np.std(imu_train, axis=0)
imu_train = (imu_train - imu_train_mean) / imu_train_std

dvl_train_mean = np.mean(dvl_train, axis=0)
dvl_train_std = np.std(dvl_train, axis=0)
dvl_train = (dvl_train - dvl_train_mean) / dvl_train_std

pressure_train_mean = np.mean(pressure_train, axis=0)
pressure_train_std = np.std(pressure_train, axis=0)
pressure_train = (pressure_train - pressure_train_mean) / pressure_train_std

ground_truth_train_mean = np.mean(ground_truth_train, axis=0)
ground_truth_train_std = np.std(ground_truth_train, axis=0)
ground_truth_train = (ground_truth_train - ground_truth_train_mean) / ground_truth_train_std

# Normalize the evaluation data using the training statistics
imu_eval = (imu_eval - imu_train_mean) / imu_train_std
dvl_eval = (dvl_eval - dvl_train_mean) / dvl_train_std
pressure_eval = (pressure_eval - pressure_train_mean) / pressure_train_std
ground_truth_eval = (ground_truth_eval - ground_truth_train_mean) / ground_truth_train_std
...
ground_truth_train = torch.tensor(ground_truth_train, dtype=torch.float32)

imu_eval = torch.tensor(imu_eval, dtype=torch.float32)
dvl_eval = torch.tensor(dvl_eval, dtype=torch.float32)
pressure_eval = torch.tensor(pressure_eval, dtype=torch.float32)
ground_truth_eval = torch.tensor(ground_truth_eval, dtype=torch.float32)
...
for epoch in range(num_epochs):
    ...
    train_loss = criterion(outputs, ground_truth_train)
    ...
    nkf.eval()
    with torch.no_grad():
        eval_outputs = nkf(imu_eval, dvl_eval, pressure_eval)
        eval_loss = criterion(eval_outputs, ground_truth_eval)
...
# Save the trained model
torch.save(nkf.state_dict(), 'nkf_single_timestep_8020split.pth')
print("Training complete. Model saved as 'nkf_single_timestep_8020split.pth'.")
...
nkf.eval()
with torch.no_grad():
    predictions = nkf(imu_eval, dvl_eval, pressure_eval).numpy()
ground_truth_eval = ground_truth_eval.numpy()

# Plot the results - 2D trajectory
plt.figure(figsize=(10, 6))
plt.plot(ground_truth_eval[:, 0], ground_truth_eval[:, 1], label='Ground Truth', linestyle='--', color='orange')
plt.plot(predictions[:, 0], predictions[:, 1], label='Predictions', color='blue')
plt.xlabel('X Position (m)')
plt.ylabel('Y Position (m)')
plt.title('2D Trajectory: Ground Truth vs Predictions (Evaluation Set)')
plt.legend()
plt.grid(True)
plt.show()

# Apply trajectory smoothing using moving average
def moving_average(data, window_size=5):
    smoothed_data = np.convolve(data, np.ones(window_size)/window_size, mode='valid')
    return smoothed_data
...
# Adjust ground truth to match the smoothed prediction length
smoothed_ground_truth_x = ground_truth_eval[window_size-1:, 0]
smoothed_ground_truth_y = ground_truth_eval[window_size-1:, 1]
smoothed_ground_truth_z = ground_truth_eval[window_size-1:, 2]

# Plot the smoothed trajectory - 2D
plt.figure(figsize=(10, 6))
plt.plot(smoothed_ground_truth_x, smoothed_ground_truth_y, label='Ground Truth', linestyle='--', color='orange')
plt.plot(smoothed_predictions_x, smoothed_predictions_y, label='Predictions (Smoothed)', color='blue')
plt.xlabel('X Position (m)')
plt.ylabel('Y Position (m)')
plt.title('Smoothed 2D Trajectory: Ground Truth vs Predictions (Evaluation Set)')
plt.legend()
plt.grid(True)
plt.show()
...
fig = plt.figure(figsize=(10, 10))
ax = fig.add_subplot(111, projection='3d')
ax.plot(smoothed_ground_truth_x, smoothed_ground_truth_y, smoothed_ground_truth_z, 
        label='Ground Truth', linestyle='--', color='orange')
ax.plot(smoothed_predictions_x, smoothed_predictions_y, smoothed_predictions_z, 
        label='Predictions (Smoothed)', color='blue')
ax.set_xlabel('X Position (m)')
ax.set_ylabel('Y Position (m)')
ax.set_zlabel('Z Position (m)')
ax.set_title('Smoothed 3D Trajectory: Ground Truth vs Predictions (Evaluation Set)')
ax.legend()
plt.show()